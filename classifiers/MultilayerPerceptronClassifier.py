...
class MultilayerPerceptronClassifier:
    ...
    def classify(self, training_data, test_data, percentage_range, metrics):
        print("Start classification with MLPClassifier")
        print("")
        ...
        for i in range(len(test_objects_without_headers)):
            ...
            train_features = tr_dataset_dataframe.iloc[:, :no_column - 1]  # Wyodrębnienie części warunkowej danych
            train_labels = tr_dataset_dataframe.iloc[:, [no_column - 1]]  # Wyodrębnienie kolumny decyzyjnej

            model = MLPClassifier(hidden_layer_sizes=self.hidden_layer_sizes, activation=self.activation, solver=self.solver, alpha=self.alpha)
